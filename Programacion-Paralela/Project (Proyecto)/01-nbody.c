...
#pragma omp parallel for
    for (i = ind_init; i < ind_final; ++i) {
        // Obtener el n�mero de hilo actual
        //int thread_id = omp_get_thread_num();
        // Obtener el n�mero total de hilos
        //int num_threads = omp_get_num_threads();

        // Imprimir el n�mero de hilo y el n�mero total de hilos.
...
int main(const int argc, const char** argv) {
  ...
  {
      /*
      * Si soy el maestro realizo la verificacion del tama�o
  ...
  /*
  * El maestro envia el tama�o de nBodies en dado caso que se ha modificado
